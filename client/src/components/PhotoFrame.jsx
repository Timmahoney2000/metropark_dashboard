...
export default function PhotoFrame({ now, photoSeconds }) {
 ...
 useEffect(() => {
    ...
    const advance = () => {
        ...
        const firstKind = orientation.current.get(first.url);
        if (firstKind === "portrait") {
            // Pull up to two more portraits into a filmstrip.
            const strip = [first];
            for (let i = 0; i < queue.current.length && strip.length < 3; ) {
                if (orientation.current.get(queue.current[i].url) === "portrait") {
                    strip.push(queue.current.splice(i, 1)[0]);
                } else i++;
            }
            setSlide({ kind: "strip", items: strip });
        } else {
            setSlide({ kind: "single", items: [first] });
        }
    };
    if (!slide) advance();
...
const noteOrientation = (e, url) => {
    const img = e.target;
    orientation.current.set(url, img.naturalHeight > img.naturalWidth ? "portrait" : "landscape");
};

if (!photos.length) {
    return (
        <div className="frame">
            <div className="empty">Waiting for photos - add some to the frame folder</div>
            <div className="clock-chip">{fmtTime(now)}</div>
        </div>
    );
}

console.log("[frame] slide:", slide);
    ...
    <div className="frame">
        {slide?.kind === "strip" ? (
            <div className="filmstrip">
                {slide.items.map((p) => (
                    <img key={p.url} src={p.url} alt="" onLoad={(e) => noteOrientation(e, p.url)} />
                ))}
                </div>
            ) : (
                slide && (
                    <img
                    key={slide.items[0].url}
                    className="showing"
